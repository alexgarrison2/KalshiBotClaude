...
_SIGMA_EARLY_DEFAULT = 3.0   # °F before 11 AM ET  (original hardcoded value)
_SIGMA_LATE_DEFAULT  = 2.0   # °F from 11 AM ET on (original hardcoded value)
_SIGMA_CUTOFF_HOUR   = 11    # ET hour that separates "morning" from "afternoon"
...
def get_calibrated_sigma(series: str, hour_et: int) -> float:
    ...
    default = (
        _SIGMA_LATE_DEFAULT if hour_et >= _SIGMA_CUTOFF_HOUR
        else _SIGMA_EARLY_DEFAULT
    )
    ...
    key = "afternoon" if hour_et >= _SIGMA_CUTOFF_HOUR else "morning"
    return float(month_data.get(key, default))
